(function () {
  ...
  let promesaPublicadorAndroid = null;
  function cargarDependenciaGlobal(src, globalName) {
    if (globalName && window[globalName]) return Promise.resolve(window[globalName]);
    return new Promise((resolve, reject) => {
      const existente = [...document.scripts].find(s => String(s.src || '').includes(src.split('?')[0]));
      const comprobar = () => {
        const value = globalName ? window[globalName] : true;
        if (value) resolve(value);
        else reject(new Error(globalName === 'JSZip' ? 'LECTOR_APK_NO_DISPONIBLE' : 'PUBLICADOR_ANDROID_NO_CARGADO'));
      };
      if (existente) {
        if (globalName && window[globalName]) return resolve(window[globalName]);
        existente.addEventListener('load', comprobar, { once:true });
        existente.addEventListener('error', () => reject(new Error('DEPENDENCIA_APK_NO_DISPONIBLE')), { once:true });
        setTimeout(() => {
      ...
      script.src = src;
      script.async = false;
      script.dataset.sgfApkDependency = globalName || '1';
      script.onload = comprobar;
      script.onerror = () => reject(new Error('DEPENDENCIA_APK_NO_DISPONIBLE'));
      document.head.appendChild(script);
    });
  }

  async function asegurarPublicadorAndroid() {
    const actual = window.SGFPublicadorAndroid;
    if (actual && typeof actual.analyzeApk === 'function' && typeof actual.publish === 'function') return actual;
    if (promesaPublicadorAndroid) return promesaPublicadorAndroid;
    promesaPublicadorAndroid = (async () => {
      if (typeof window.JSZip === 'undefined') {
        await cargarDependenciaGlobal('jszip.min.js?v=4.3.25-apkpub2', 'JSZip');
      }
      let pub = window.SGFPublicadorAndroid;
      if (!pub || typeof pub.analyzeApk !== 'function' || typeof pub.publish !== 'function') {
        await cargarDependenciaGlobal('sgf-publicador-android.js?v=4.3.25-apkpub2', 'SGFPublicadorAndroid');
        pub = window.SGFPublicadorAndroid;
      }
      if (!pub || typeof pub.analyzeApk !== 'function' || typeof pub.publish !== 'function') {
        throw new Error('PUBLICADOR_ANDROID_NO_CARGADO');
      }
      return pub;
    })().catch(error => {
      promesaPublicadorAndroid = null;
      throw error;
    });
    return promesaPublicadorAndroid;
  }
  ...
  const dependenciasCacheSeccion = Object.freeze({
    ...
    geofence:{ actions:['resumenGeocerca'], resources:['drivers','users'] },
  ...
  const navGroups = [
    ...
    ['ADMINISTRACIÓN', [
      ['connections','◎','Conexiones en línea'], ['geofence','⌖','Geocerca'], ['users','♚','Usuarios'], ['company','🏢','Empresa'], ['reports','▥','Reportes'], ['audit','☷','Auditoría'], ['appUpdates','⬆','Actualización de Aplicación'], ['settings','⚒','Configuración']
    ]]
  ];
  ...
  const resourceFields = {
    ...
    users: {
      title:'Usuario', eyebrow:'SEGURIDAD', fields:[
        ...
        ['ROL_ID','Rol','select',[['ROL-ADMIN','Administrador'],['ROL-GERENCIA','Gerencia'],['ROL-OPERADOR','Operador'],['ROL-CONDUCTOR','Conductor']]],
        ...
        ['SUPERVISOR_USUARIO_ID','Supervisor asignado','userSelect',false],
        ['DOMICILIO_DIRECCION','Dirección de domicilio','text',false],['DOMICILIO_LATITUD','Latitud domicilio','number',false],['DOMICILIO_LONGITUD','Longitud domicilio','number',false],['DOMICILIO_RADIO_METROS','Radio geocerca domicilio (m)','number',false],
        ['TURNO_INICIO','Inicio de turno (HH:MM)','text',false],['TURNO_FIN','Fin de turno (HH:MM)','text',false],['TURNO_DIAS','Días turno (1=Lun…7=Dom)','text',false]
      ]
    }
  };
  ...
  const labels = {
    ...
    notifications:'Notificaciones',documents:'Documentos del conductor',history:'Historial',alerts:'Alertas',connections:'Conexiones en línea',geofence:'Geocerca',users:'Usuarios',reports:'Reportes',audit:'Auditoría',company:'Empresa',appUpdates:'Actualización de Aplicación',settings:'Configuración'
  };
  ...
  const permissionCatalog = Object.freeze([
    ...
    ['REPORTES','Reportes'],['BITACORA','Auditoría'],['GEOCERCA','Geocerca'],['CONFIGURACION','Configuración'],['QR','QR'],['RUTAS','Rutas'],
  ...
  function puedeCierreExcepcional(){return hasPermission('OPERACIONES','CIERRE_EXCEPCIONAL')&&['ROL-ADMIN','ROL-GERENCIA','ROL-OPERADOR','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||''));}
  function puedeReenviarAlertaAsignacion(){return ['ROL-ADMIN','ROL-GERENCIA','ROL-OPERADOR','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())&&hasPermission('NOTIFICACIONES','ENVIAR');}
  ...
  function puedeRevisarDocumentos(){return ['ROL-ADMIN','ROL-GERENCIA','ROL-OPERADOR','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').trim().toUpperCase());}
  ...
  function notificacionesSilenciadasServidor(){
    const rol=String(currentUser?.ROL_ID||currentUser?.ROL_NOMBRE||'').toUpperCase();
    if(!['ROL-GERENCIA','GERENCIA'].includes(rol))return false;
    return ['SI','TRUE','1'].includes(String(currentUser?.NOTIFICACIONES_SILENCIADAS??currentUser?.notificaciones_silenciadas??'NO').toUpperCase());
  }
  function avisosEmergentesActivos(){
    if(notificacionesSilenciadasServidor())return false;
    if(!['ROL-ADMIN','ROL-GERENCIA','ROL-OPERADOR','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase()))return true;
    try{return localStorage.getItem(claveAvisosEmergentes())!=='NO';}
    catch(_){return true;}
  }
  function esGerenciaActual(){const rol=String(currentUser?.ROL_ID||currentUser?.ROL_NOMBRE||'').toUpperCase();return ['ROL-GERENCIA','GERENCIA'].includes(rol);}
  function esOperadorActual(){const rol=String(currentUser?.ROL_ID||currentUser?.ROL_NOMBRE||'').toUpperCase();return ['ROL-OPERADOR','ROL-SUPERVISOR','OPERADOR','SUPERVISOR'].includes(rol);}
  function puedeTextoLibreNexo(){return esAdministrador()&&!esOperadorActual()||esGerenciaActual();}
  ...
  function translateError(error) {
    ...
    const messages = {
      ...
      PUBLICADOR_ANDROID_NO_CARGADO:'No se pudo iniciar el analizador de APK. Recargue la Web con Ctrl + F5 e inténtelo nuevamente.', LECTOR_APK_NO_DISPONIBLE:'No se pudo iniciar el lector interno de APK. Recargue la Web con Ctrl + F5 e inténtelo nuevamente.', DEPENDENCIA_APK_NO_DISPONIBLE:'No se pudieron cargar los componentes necesarios para analizar la APK. Verifique que jszip.min.js y sgf-publicador-android.js estén publicados junto con la Web.',
  ...
  async function mostrarAccesoSuspendidoWeb(correo,codigo){
    let contacto={};try{contacto=await api.request('contactoSupervisorAcceso',{data:{CORREO:correo},correo,cache:false});}catch(_){}
    const supervisor=contacto?.supervisor||contacto?.SUPERVISOR||{},telefono=String(supervisor.TELEFONO||supervisor.telefono||'').replace(/[^0-9]/g,'');
    $('#modalEyebrow').textContent='ACCESO SUSPENDIDO';$('#modalTitle').textContent=codigo==='USUARIO_INACTIVO'?'Cuenta inactiva':'Cuenta bloqueada';
    const texto=codigo==='USUARIO_INACTIVO'?'Su cuenta está inactiva. Debe comunicarse con su supervisor asignado antes de volver a ingresar.':'Su acceso está bloqueado. Comuníquese con su supervisor asignado para revisar el estado de su cuenta.';
    $('#modalBody').innerHTML=`<section class="access-suspended-view"><div class="access-suspended-icon">!</div><h3>${esc(texto)}</h3><div class="tracking-notice warning full"><i>♙</i><div><b>${esc(supervisor.NOMBRE||supervisor.nombre||'Supervisor asignado')}</b><span>${esc(supervisor.TELEFONO||supervisor.telefono||'Teléfono no registrado')}</span></div></div><div class="form-actions"><button class="btn soft" type="button" data-cancel-modal>Cerrar</button>${telefono?`<a class="btn whatsapp" target="_blank" rel="noopener" href="https://wa.me/${telefono}?text=${encodeURIComponent('Hola, necesito ayuda con mi acceso al Sistema de Gestión de Flotas. Mi correo es '+correo)}">Abrir WhatsApp</a>`:''}</div></section>`;openModal();$('[data-cancel-modal]',$('#modalBody'))?.addEventListener('click',closeModal);
  }
  ...
  async function handleLogin(event) {
    ...
    await conCargaBoton(button,'Verificando…',async()=>{
      ...
      } catch (error) { const code=String(error?.code||error?.message||'').toUpperCase();if(code.includes('USUARIO_INACTIVO'))await mostrarAccesoSuspendidoWeb(String(form.get('correo')||''),'USUARIO_INACTIVO');else if(code.includes('USUARIO_BLOQUEADO'))await mostrarAccesoSuspendidoWeb(String(form.get('correo')||''),'USUARIO_BLOQUEADO');else toast('Acceso denegado',translateError(error),'error'); }
    });
  }
  ...
  function buildNav() {
    ...
    let html='';const destinosMenu=new Set();
    navGroups.forEach(([group,items]) => {
      const visible = items.filter(([id]) => {if(destinosMenu.has(id))return false;if(id==='geofence'){const rol=String(currentUser?.ROL_ID||'').toUpperCase();if(!['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA'].includes(rol))return false;}const ok=hasPermission(navPermission[id]||'PANEL_PRINCIPAL','LEER');if(ok)destinosMenu.add(id);return ok;});
    ...
    const task=(async()=>{
      try{
        ...
        if(currentSection===section){
          ...
          if(section==='geofence')setTimeout(initGeocercaMap,60);
  ...
  const renderers = {
    async dashboard() {
      ...
      const headingActions=`${esGerenciaActual()?`<label class="assignment-alert-switch compact" title="Las notificaciones seguirán llegando a la bandeja, pero sin interrupciones emergentes"><input type="checkbox" data-gerencia-silencio ${notificacionesSilenciadasServidor()?'checked':''}><i></i><span><b>Silenciar avisos emergentes</b></span></label>`:''}<button class="icon-button dashboard-refresh-circle" type="button" data-sync aria-label="Actualizar panel principal ahora" title="Actualizar panel principal">↻</button>${hasPermission('RUTAS','CREAR')?'<button class="btn primary" data-new-route>＋ Asignar ruta</button>':''}`;
    ...
    async geofence(){return renderGeocerca();},
  ...
  function puedeVerLineaTiempoRuta(){return ['ROL-ADMIN','ROL-GERENCIA','ROL-OPERADOR','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())&&hasPermission('RUTAS','LEER');}
  function routeTimelineMarkup(route){
    ...
    const kpi=route?.KPI_TRAZABILIDAD||{};
    const eventTime=value=>{const ms=new Date(value||0).getTime();return Number.isFinite(ms)?ms:0;};
    const byEvent=Object.fromEntries(events.filter(event=>!['REASIGNADA','REASIGNACION_ACEPTADA','REASIGNACION_ANTERIOR_NOTIFICADA'].includes(String(event.EVENTO||'').toUpperCase())).map(event=>[String(event.EVENTO||'').toUpperCase(),event]));
    const stages=[
    ...
    ].map(stage=>{const event=byEvent[stage.event]||{},date=kpi[stage.date]||event.FECHA_EVENTO||(stage.event==='ASIGNADA'?(route.FECHA_ASIGNACION||route.CREADO_EN):stage.event==='INICIADA'?route.FECHA_INICIO:stage.event==='COMPLETADA'&&textoEstadoKpi(route.ESTADO)==='completada'?route.FECHA_FIN:'')||'';return{...stage,event,date,sort:eventTime(date)};});
    const reassignmentEvents=events.filter(event=>['REASIGNADA','REASIGNACION_ACEPTADA','REASIGNACION_ANTERIOR_NOTIFICADA'].includes(String(event.EVENTO||'').toUpperCase())).map(event=>({event:String(event.EVENTO||'').toUpperCase(),eventData:event,date:event.FECHA_EVENTO||event.CREADO_EN||'',sort:eventTime(event.FECHA_EVENTO||event.CREADO_EN)}));
    const ordered=[...stages,...reassignmentEvents].sort((a,b)=>{if(!a.sort&&!b.sort)return 0;if(!a.sort)return 1;if(!b.sort)return-1;return a.sort-b.sort;});
    const card=(item,index)=>{
      const event=item.eventData||item.event||{};
      const eventName=typeof item.event==='string'?item.event:item.event?.EVENTO||item.event;
      const isReassign=['REASIGNADA','REASIGNACION_ACEPTADA','REASIGNACION_ANTERIOR_NOTIFICADA'].includes(String(eventName||'').toUpperCase());
      const date=item.date||'';
      const done=Boolean(date);
      const eventDate=done?new Date(date):null,validDate=eventDate&&!Number.isNaN(eventDate.getTime());
      const dateText=validDate?new Intl.DateTimeFormat('es-CL',{day:'2-digit',month:'2-digit',year:'numeric'}).format(eventDate):done?String(date):'Fecha pendiente';
      const clockText=validDate?new Intl.DateTimeFormat('es-CL',{hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'}).format(eventDate):done?'Hora registrada':'Hora pendiente';
      if(isReassign){
        const type=String(eventName||'').toUpperCase(),from=event.CONDUCTOR_ANTERIOR_NOMBRE||event.CONDUCTOR_ANTERIOR_ID||route.REASIGNADA_DESDE_CONDUCTOR_ID||'Conductor anterior',to=event.CONDUCTOR_NUEVO_NOMBRE||event.CONDUCTOR_NUEVO_ID||route.CONDUCTOR_NOMBRE||route.CONDUCTOR_ID||'Nuevo conductor',actor=event.USUARIO_NOMBRE||event.REASIGNADO_POR_NOMBRE||'',role=event.ACTOR_ROL||event.REASIGNADO_POR_ROL||'',reason=event.MOTIVO_REASIGNACION||route.MOTIVO_REASIGNACION||'';
        const label=type==='REASIGNADA'?'Ruta reasignada':type==='REASIGNACION_ACEPTADA'?'Reasignación aceptada':'Conductor anterior informado';
        const main=type==='REASIGNADA'?`${from} → ${to}`:type==='REASIGNACION_ACEPTADA'?`${to} aceptó continuar la ruta`:`${from} confirmó el aviso de retiro`;
        return `<div class="completed route-reassignment-stage"><i>↻</i><span><b>${esc(label)}</b><small class="route-stage-date">${esc(dateText)}</small><small class="route-stage-clock">${esc(clockText)}</small><strong class="route-reassignment-transfer">${esc(main)}</strong>${reason?`<small class="route-reassignment-reason">Motivo: ${esc(reason)}</small>`:''}${actor?`<em>Registrado por ${esc(actor)}${role?` · ${esc(role.replace(/^ROL-/i,'').replace('SUPERVISOR','OPERADOR'))}`:''}</em>`:''}</span></div>`;
      }
      const stage=item,eventObj=item.event||{},actor=kpi[stage.actor]||eventObj.USUARIO_NOMBRE||'',seconds=stage.duration?Number(kpi[stage.duration]??eventObj.DURACION_DESDE_ANTERIOR_SEGUNDOS??0):0,total=stage.event==='COMPLETADA'?Number(kpi.TIEMPO_TOTAL_CICLO_SEGUNDOS||0):0;
      return `<div class="${done?'completed':'pending'}"><i>${done?'✓':index+1}</i><span><b>${stage.label}</b><small class="route-stage-date">${esc(dateText)}</small><small class="route-stage-clock">${esc(clockText)}</small><small class="route-stage-time">${done?(stage.event==='ASIGNADA'?'Inicio de la trazabilidad':seconds?`${stage.help}: ${formatRouteElapsed(seconds)}`:'Duración pendiente'):'Duración pendiente'}</small>${done&&total?`<small class="route-stage-total">Ciclo total: ${formatRouteElapsed(total)}</small>`:''}${actor?`<em>${esc(actor)}</em>`:''}</span></div>`;
    };
    return `<div class="route-timeline ${reassignmentEvents.length?'has-reassignment':''}" aria-label="Trazabilidad de la ruta">${ordered.map(card).join('')}</div>`;
  }

  function puedeVerTrazabilidadRutas(){
    return ['ROL-ADMIN','ROL-GERENCIA','ROL-OPERADOR','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())&&hasPermission('RUTAS','LEER');
  }
  ...
  function canSelectGpsVehicles() {
    return ['ROL-ADMIN','ROL-GERENCIA','ROL-OPERADOR','ROL-SUPERVISOR'].includes(currentUser?.ROL_ID);
  }
  ...
  const geocercaFiltros={desde:'',hasta:'',conductorId:''};
  function geocercaFechaIso(date){return date.toISOString().slice(0,10);}
  function geocercaPeriodo(periodo='MES'){
    const now=new Date(),desde=new Date(now);if(periodo==='DIA')desde.setHours(0,0,0,0);else if(periodo==='ANO')desde.setMonth(0,1);else desde.setDate(1);
    return {desde:geocercaFechaIso(desde),hasta:geocercaFechaIso(now)};
  }
  async function renderGeocerca(){
    const rol=String(currentUser?.ROL_ID||'').toUpperCase();if(!['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA'].includes(rol))throw new Error('PERMISO_DENEGADO');
    if(!geocercaFiltros.desde){const p=geocercaPeriodo('MES');Object.assign(geocercaFiltros,p);}
    const data=await api.request('resumenGeocerca',{data:{FECHA_DESDE:geocercaFiltros.desde,FECHA_HASTA:geocercaFiltros.hasta,CONDUCTOR_ID:geocercaFiltros.conductorId},cache:false});
    window.__geocercaData=data||{};const rows=Array.isArray(data?.rows)?data.rows:[],summary=data?.summary||data?.resumen||{};
    const drivers=Array.isArray(data?.conductores)?data.conductores:(Array.isArray(data?.drivers)?data.drivers:[]);
    const opts=drivers.map(d=>`<option value="${esc(d.ID)}" ${String(geocercaFiltros.conductorId)===String(d.ID)?'selected':''}>${esc(d.NOMBRE||d.ID)}</option>`).join('');
    const tableRows=rows.map(r=>`<tr><td><strong>${esc(r.CONDUCTOR_NOMBRE||r.USUARIO_NOMBRE||'Conductor')}</strong><span class="muted">${esc(r.DIRECCION_DOMICILIO||r.DIRECCION||'Domicilio')}</span></td><td>${fmtDate(r.FECHA_HORA||r.CREADO_EN,true)}</td><td>${number(Number(r.DISTANCIA_METROS||0).toFixed(0))} m</td><td>${esc(`${r.TURNO_INICIO||'--:--'}–${r.TURNO_FIN||'--:--'}`)}</td><td><button class="btn soft small" type="button" data-geocerca-focus="${esc(r.LATITUD)},${esc(r.LONGITUD)}">Ver mapa</button></td></tr>`).join('');
    return heading('CONTROL DE GEOCERCA','Geocerca','Registro silencioso de visitas al domicilio durante el turno laboral. Acceso exclusivo de SYSADMIN, Administración y Gerencia.')+
      `<div class="kpi-grid">${metric('⌂','Visitas registradas',Number(summary.total||rows.length),'Período filtrado')}${metric('♙','Conductores',Number(summary.conductores||new Set(rows.map(r=>r.CONDUCTOR_ID)).size),'Con eventos')}${metric('◷','Hoy',Number(summary.hoy||0),'Eventos de hoy')}${metric('▥','Mes',Number(summary.mes||rows.length),'Eventos del mes')}</div>`+
      `<article class="card geocerca-filter-card"><form id="geocercaFilterForm" class="form-grid"><label class="field"><span>Desde</span><input type="date" name="FECHA_DESDE" value="${esc(geocercaFiltros.desde)}"></label><label class="field"><span>Hasta</span><input type="date" name="FECHA_HASTA" value="${esc(geocercaFiltros.hasta)}"></label><label class="field"><span>Conductor</span><select name="CONDUCTOR_ID"><option value="">Todos</option>${opts}</select></label><div class="form-actions"><button class="btn soft" type="button" data-geocerca-periodo="DIA">Día</button><button class="btn soft" type="button" data-geocerca-periodo="MES">Mes</button><button class="btn soft" type="button" data-geocerca-periodo="ANO">Año</button><button class="btn primary" type="submit">Aplicar filtros</button></div></form></article>`+
      `<div class="gps-layout geocerca-layout"><article class="card map-card" id="mapCard"><div id="fleetMap" class="fleet-map"></div><div class="map-toolbar"><span class="gps-live"><i></i> Geocerca domicilio</span><span class="muted">El marcador identifica al conductor; cuando existe foto de perfil se muestra en el detalle.</span></div></article><article class="card"><div class="card-header"><div><h3>Eventos del período</h3><p>${rows.length} registro(s)</p></div></div>${tableRows?table(['Conductor / domicilio','Fecha y hora','Distancia','Turno','Mapa'],tableRows):empty('⌂','Sin visitas registradas','No hay ingresos al domicilio dentro del turno para los filtros seleccionados.')}</article></div>`;
  }
  async function initGeocercaMap(){
    const cont=$('#fleetMap');if(!cont||currentSection!=='geofence')return;await asegurarComponenteMapa();const visible=await esperarTamanoMapa(cont,50);if(!visible||currentSection!=='geofence')return;mapaFlota?.eliminar?.();mapaFlota=new window.MapaFlotas(cont,{centro:config.CENTRO_MAPA,nivel:config.NIVEL_ACERCAMIENTO_MAPA});
    const rows=Array.isArray(window.__geocercaData?.rows)?window.__geocercaData.rows:[];
    const marcadores=rows.filter(r=>Number.isFinite(Number(r.LATITUD))&&Number.isFinite(Number(r.LONGITUD))).map(r=>{const foto=String(r.FOTO_PERFIL_URL||'');const detalle=`${foto?`<img src="${esc(foto)}" alt="" style="width:54px;height:54px;border-radius:50%;object-fit:cover;float:left;margin:0 10px 8px 0">`:''}<b>${esc(r.CONDUCTOR_NOMBRE||r.USUARIO_NOMBRE||'Conductor')}</b><span><strong>Domicilio:</strong> ${esc(r.DIRECCION_DOMICILIO||r.DIRECCION||'No informada')}</span><span><strong>Fecha:</strong> ${esc(fmtDate(r.FECHA_HORA||r.CREADO_EN,true))}</span><span><strong>Distancia:</strong> ${number(Number(r.DISTANCIA_METROS||0).toFixed(0))} m</span>`;return{id:String(r.ID||r.MARCADOR_ID||crypto.randomUUID()),latitud:Number(r.LATITUD),longitud:Number(r.LONGITUD),nombre:r.CONDUCTOR_NOMBRE||r.USUARIO_NOMBRE||'Conductor',direccion:r.DIRECCION_DOMICILIO||r.DIRECCION||'',activo:true,imagen:foto,detalle};});
    mapaFlota.actualizarMarcadores(marcadores,true);setTimeout(()=>mapaFlota?.redibujar?.(),200);
  }
  ...
  let ultimoResumenReportesKpi=null,ultimaRespuestaReportesKpi=null,secuenciaReportesKpi=0,temporizadorFiltrosReportesKpi=null;
  function resumenVisualFiltrosReportesKpi(){
    const form=$('#kpiFilterForm');if(!form)return 'Todo el historial autorizado';
    const partes=[],desde=String(form.elements.FECHA_DESDE?.value||'').trim(),hasta=String(form.elements.FECHA_HASTA?.value||'').trim();
    const conductor=form.elements.CONDUCTOR_ID?.selectedOptions?.[0],vehiculo=form.elements.VEHICULO_ID?.selectedOptions?.[0];
    if(desde)partes.push(`Desde ${desde}`);if(hasta)partes.push(`Hasta ${hasta}`);
    if(form.elements.CONDUCTOR_ID?.value)partes.push(`Conductor: ${conductor?.textContent?.trim()||form.elements.CONDUCTOR_ID.value}`);
    if(form.elements.VEHICULO_ID?.value)partes.push(`Vehículo: ${vehiculo?.textContent?.trim()||form.elements.VEHICULO_ID.value}`);
    return partes.length?partes.join(' · '):'Todo el historial autorizado';
  }
  function pintarEstadoFiltrosReportesKpi(cargando=false){
    const node=$('[data-kpi-filter-status]');if(!node)return;
    node.classList.toggle('is-loading',Boolean(cargando));
    node.innerHTML=`<i>${cargando?'↻':'✓'}</i><span><b>${cargando?'Actualizando indicadores…':'Indicadores sincronizados con filtros'}</b><small>${esc(resumenVisualFiltrosReportesKpi())}</small></span>`;
  }
  ...
  function reporteLineaTendencia(series=[]){
    ...
    const width=660,height=230,padX=38,padY=30,single=rows.length===1,max=Math.max(1,...rows.map(row=>row.total)),step=rows.length>1?(width-padX*2)/(rows.length-1):0;
    const points=rows.map((row,index)=>({x:single?width/2:padX+step*index,y:single?height/2:height-padY-(row.total/max)*(height-padY*2),...row}));
    ...
    const seriesMarkup=single?`<line class="report-line-single-guide" x1="${padX}" y1="${height/2}" x2="${width-padX}" y2="${height/2}"/>`:`<polygon class="report-line-area" points="${area}"/><polyline class="report-line-path" points="${polyline}" fill="none"/>`;
    return `<div class="report-line-chart${single?' is-single':''}" role="group" aria-label="Línea de tendencia operacional"><svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-hidden="false"><defs><linearGradient id="reportTrendArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="currentColor" stop-opacity=".24"/><stop offset="100%" stop-color="currentColor" stop-opacity="0"/></linearGradient></defs><g class="report-line-grid"><line x1="${padX}" y1="${padY}" x2="${width-padX}" y2="${padY}"/><line x1="${padX}" y1="${height/2}" x2="${width-padX}" y2="${height/2}"/><line x1="${padX}" y1="${height-padY}" x2="${width-padX}" y2="${height-padY}"/></g>${seriesMarkup}${points.map((p,index)=>`<g class="report-line-point" tabindex="0" role="button" data-report-detail="${esc(`${p.label}: ${number(p.total)} operaciones`)}" aria-label="${esc(`${p.label}: ${number(p.total)} operaciones`)}"><circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="7"/><text class="report-line-value" x="${p.x.toFixed(1)}" y="${Math.max(16,p.y-13).toFixed(1)}" text-anchor="middle">${number(p.total)}</text><text class="report-line-label" x="${p.x.toFixed(1)}" y="${height-8}" text-anchor="middle">${esc(p.label.slice(0,10))}</text></g>`).join('')}</svg></div>`;
  }
  ...
  async function pintarKpisReportes(recargar=false){
    ...
    if(recargar){const solicitud=++secuenciaReportesKpi;pintarEstadoFiltrosReportesKpi(true);target.innerHTML='<article class="card loading-state"><span class="spinner"></span><b>Calculando indicadores según los filtros…</b><small>Todos los KPI, gráficos y tablas se recalculan con el mismo alcance.</small></article>';try{const respuesta=await api.request('reportsKpiSummary',{data:filtrosReportesKpi(),cache:false});if(solicitud!==secuenciaReportesKpi||currentSection!=='reports')return;ultimaRespuestaReportesKpi=respuesta;ultimoResumenReportesKpi=respuesta?.resumen||{};pintarEstadoFiltrosReportesKpi(false);}catch(error){if(solicitud!==secuenciaReportesKpi)return;pintarEstadoFiltrosReportesKpi(false);target.innerHTML=`<article class="module-error"><b>No fue posible calcular los indicadores</b><span>${esc(translateError(error))}</span><button class="btn primary" data-kpi-retry>Reintentar</button></article>`;$('[data-kpi-retry]',target)?.addEventListener('click',()=>pintarKpisReportes(true));return;}}
  ...
  async function renderOficinaVirtual(){
    ...
    const canConfigure=Boolean(data.puedeConfigurar)&&hasPermission('OFICINA_VIRTUAL','CONFIGURAR'),canUpload=hasPermission('DOCUMENTOS','CARGAR_PROPIO'),canReports=hasPermission('OFICINA_VIRTUAL','GENERAR_REPORTE'),canFreeText=puedeTextoLibreNexo();
    ...
    const suggestions=(currentUser?.ROL_ID==='ROL-CONDUCTOR'||esOperadorActual())?['¿Tengo vehículo asignado?','Muéstrame mi licencia de conducir','Muestrame los Documento del Vehiculo','Actualizar aplicación','¿Qué alertas tengo?','¿Qué tengo pendiente?','¿Qué mantenciones están próximas?']:['¿Tengo vehículo asignado?','Muéstrame mi licencia de conducir','Muestrame los Documento del Vehiculo','Actualizar aplicación','¿Qué alertas tengo?','¿Hay excesos de velocidad?'];
    ...
    return heading('CENTRO INTELIGENTE DE GESTIÓN','NEXO IA','Inteligencia operacional personalizada por rol, documentos en el chat, alertas y datos reales del sistema.',`<button class="btn soft" type="button" data-office-review>${reviewLabel}</button>${canReports?'<button class="btn primary" type="button" data-office-generate-report>▤ Reporte de salud</button>':''}`)+
      ...
      `<section class="office-layout"><article class="card office-chat-card"><div class="card-header"><div><h3>Pregúntale a NEXO IA</h3><p>${canFreeText?'Consulta libre dentro de tus permisos.':'Utiliza únicamente las consultas rápidas autorizadas para tu perfil.'}</p></div><span class="status ok">● NEXO IA EN LÍNEA</span></div>${data.rutaActivaPendiente?`<div class="tracking-notice warning full"><i>!</i><div><b>Ruta activa pendiente de culminar</b><span>${esc(data.rutaActivaPendiente.mensaje||data.rutaActivaPendiente.MENSAJE||'Debes finalizar tu ruta activa antes de cerrar el ciclo operacional.')}</span></div></div>`:''}<div class="office-chat" id="officeChat">${markupConversacionOficinaVirtual()}</div><div class="office-suggestions">${suggestions.map(text=>`<button type="button" data-office-suggestion="${esc(text)}">${esc(text)}</button>`).join('')}</div>${canFreeText?`<form class="office-form" id="officeForm"><textarea name="MENSAJE" rows="2" maxlength="1200" required placeholder="Ejemplo: Muéstrame los documentos del vehículo ABCD12"></textarea><button class="btn primary" type="submit">Enviar</button></form>`:'<p class="helper nexo-restricted-input"><b>Consulta guiada:</b> por seguridad, Conductor y Operador usan únicamente los botones predeterminados.</p>'}<p class="helper">La IA trabaja con datos del propio sistema. No solicita contraseñas ni expone la clave administrativa.</p></article>`+
  ...
  function canFreeTextNexoPayload(){return puedeTextoLibreNexo();}
  ...
  async function enviarConsultaOficinaVirtual(text,button){
    ...
    try{
      const result=await api.request('officeAsk',{data:{MENSAJE:question,CONSULTA_PREDETERMINADA:canFreeTextNexoPayload(question)?'NO':'SI'}});
  ...
  function fuelActionMarkup(row,authorization){
    ...
    else if(['ROL-OPERADOR','ROL-SUPERVISOR'].includes(currentUser.ROL_ID)&&hasPermission('COMBUSTIBLE','SOLICITAR_ELIMINACION')){
  ...
  async function configurarSilencioNotificacionesWeb(input){
    if(!esGerenciaActual())return;const activo=Boolean(input.checked);input.disabled=true;
    try{const result=await api.request('configurarSilencioNotificaciones',{data:{ACTIVO:activo?'SI':'NO'}});const user=result?.user||result?.usuario||{...currentUser,NOTIFICACIONES_SILENCIADAS:activo?'SI':'NO'};currentUser={...currentUser,...user,NOTIFICACIONES_SILENCIADAS:activo?'SI':'NO'};const auth=api.getAuth();api.setAuth({...auth,user:currentUser});toast(activo?'Avisos emergentes silenciados':'Avisos emergentes activados',activo?'Las notificaciones seguirán acumulándose en la bandeja sin interrumpir la pantalla.':'Los avisos emergentes volverán a mostrarse normalmente.');}
    catch(error){input.checked=!activo;toast('No se pudo cambiar la preferencia',translateError(error),'error');}
    finally{input.disabled=false;}
  }
  ...
  function bindSection() {
    ...
    $('[data-gerencia-silencio]')?.addEventListener('change',event=>configurarSilencioNotificacionesWeb(event.currentTarget));
    ...
    const geofenceForm=$('#geocercaFilterForm');if(geofenceForm)geofenceForm.addEventListener('submit',event=>{event.preventDefault();const d=Object.fromEntries(new FormData(geofenceForm).entries());geocercaFiltros.desde=d.FECHA_DESDE||'';geocercaFiltros.hasta=d.FECHA_HASTA||'';geocercaFiltros.conductorId=d.CONDUCTOR_ID||'';go('geofence',{force:true});});
    $$('[data-geocerca-periodo]').forEach(btn=>btn.addEventListener('click',()=>{Object.assign(geocercaFiltros,geocercaPeriodo(btn.dataset.geocercaPeriodo));go('geofence',{force:true});}));
    $$('[data-geocerca-focus]').forEach(btn=>btn.addEventListener('click',()=>{const [lat,lng]=String(btn.dataset.geocercaFocus||'').split(',').map(Number);if(Number.isFinite(lat)&&Number.isFinite(lng))mapaFlota?.establecerVista(lat,lng,18);}));
    ...
    $$('[data-edit]').forEach(btn=>btn.addEventListener('click',()=>{const [resource,id]=btn.dataset.edit.split(':');if(['alerts','notifications'].includes(resource)){toast('Edición deshabilitada','Las alertas y notificaciones son registros de trazabilidad y no se editan.','warning');return;}openResourceModal(resource,registroFormulario(resource,id),id);}));
    ...
    enlazarAvisosConexiones($('#content')||document);
    enlazarDesconexionUsuariosConectados($('#connectionsResults')||document);
    const kpiForm=$('#kpiFilterForm');if(kpiForm&&kpiForm.dataset.reportFiltersBound!=='1'){
      kpiForm.dataset.reportFiltersBound='1';
      const recargar=()=>{if(temporizadorFiltrosReportesKpi){clearTimeout(temporizadorFiltrosReportesKpi);temporizadorFiltrosReportesKpi=null;}return pintarKpisReportes(true);};
      const programar=(espera=180)=>{pintarEstadoFiltrosReportesKpi(true);if(temporizadorFiltrosReportesKpi)clearTimeout(temporizadorFiltrosReportesKpi);temporizadorFiltrosReportesKpi=setTimeout(()=>{temporizadorFiltrosReportesKpi=null;if(currentSection==='reports')pintarKpisReportes(true);},espera);};
      kpiForm.addEventListener('submit',event=>{event.preventDefault();recargar();});
      ['CONDUCTOR_ID','VEHICULO_ID'].forEach(name=>kpiForm.elements[name]?.addEventListener('change',()=>programar(80)));
      ['FECHA_DESDE','FECHA_HASTA'].forEach(name=>kpiForm.elements[name]?.addEventListener('change',()=>programar(120)));
      $('[data-kpi-all]',kpiForm)?.addEventListener('click',()=>{kpiForm.elements.FECHA_DESDE.value='';kpiForm.elements.FECHA_HASTA.value='';kpiForm.elements.CONDUCTOR_ID.value='';kpiForm.elements.VEHICULO_ID.value='';recargar();});
      ...
      pintarEstadoFiltrosReportesKpi(false);pintarKpisReportes(false);
    }
  ...
  function pintarModalRecurso(resource,record,token) {
    if(token!==secuenciaModal)return;if(['alerts','notifications'].includes(resource)&&record){toast('Edición deshabilitada','Las alertas y notificaciones no se pueden editar.','warning');closeModal();return;}
    ...
    const controls=definition.fields.map(([name,label,type,option])=>{
      ...
      else {const autoDoc=resource==='documents'&&['ASOCIADO_ID','CORREO_ASOCIADO'].includes(name),home=resource==='users'&&name==='DOMICILIO_DIRECCION';control=`<input name="${name}" type="${type}" value="${esc(current)}" ${required?'required':''} ${autoDoc?'readonly':''} ${home?'data-address-autocomplete data-lat-target="DOMICILIO_LATITUD" data-lng-target="DOMICILIO_LONGITUD" placeholder="Escriba calle, número y comuna"':''}>`; }
    ...
    const avisoOperador=resource==='documents'&&['ROL-OPERADOR','ROL-SUPERVISOR'].includes(currentUser.ROL_ID)&&!record?`<div class="tracking-notice active full"><i>○</i><div><b>Revisión administrativa obligatoria</b><span>Los documentos cargados por Operadores quedan pendientes hasta que un Operador, Administrador o Gerencia los apruebe.</span></div></div>`:'';
    ...
    const resourceForm=$('#resourceForm');enlazarCalendarios(resourceForm);bindAddressAutocomplete(resourceForm);resourceForm.addEventListener('submit',event=>saveResource(event,resource,record?.ID));if(resource==='documents'){enlazarCargaArchivo(resourceForm,'documents');const reviewValue=record?documentReviewState(record):(esAdministrador()?'Aprobado':'Pendiente de revisión');if(resourceForm.elements.ESTADO_REVISION)resourceForm.elements.ESTADO_REVISION.value=reviewValue;resourceForm.querySelector('[data-modal-approve-document]')?.addEventListener('click',()=>reviewDocument(record.ID,'APROBAR',resourceForm.querySelector('[data-modal-approve-document]')));resourceForm.querySelector('[data-modal-reject-document]')?.addEventListener('click',()=>reviewDocument(record.ID,'RECHAZAR',resourceForm.querySelector('[data-modal-reject-document]')));}
  ...
  async function saveResource(event,resource,id){
    ...
    await conCargaBoton(button,form._driveUploadPromise?'Esperando archivo…':'Guardando…',async()=>{
      try{
        ...
        if(resource==='documents'&&!id&&esAdministrador())toast('Documento aprobado','Quedó guardado y aprobado automáticamente.');else if(resource==='documents'&&!id&&['ROL-CONDUCTOR','ROL-OPERADOR','ROL-SUPERVISOR'].includes(currentUser.ROL_ID))toast('Documento enviado','Quedó pendiente de aprobación por Operador, Administración o Gerencia.');
