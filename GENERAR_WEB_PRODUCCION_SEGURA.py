...
htmls=['index.html','main.html','panel-principal.html','oficina-virtual.html','rutas.html','checkin-vehicular.html','operaciones.html','ubicacion-tiempo-real.html','geocerca.html','notificaciones.html','vehiculos.html','conductores.html','checkin-aprobaciones.html','checkin-historial.html','mantenciones.html','combustible.html','documentos.html','historial.html','alertas.html','conexiones-en-linea.html','usuarios.html','empresa.html','reportes.html','auditoria.html','configuracion.html','actualizaciones-app.html']
css=['acceso.css','estilos.css','responsive.css','interfaz-moderna.css','menu-principal.css','ajustes-visuales-modulos.css','reportes-estable.css']
assets=['logo.svg','Plantilla_Importacion_Vehiculos.xlsx','Plantilla_Importacion_Conductores.xlsx','Plantilla_Importacion_Documentos.xlsx','compatibilidad-web-4318.js','jszip.min.js','sgf-publicador-android.js']
...
bundles=[pack(['configuracion.js','conexion.js','acceso.js'],'sgf-login.4328.js','LOGIN'),pack(['configuracion.js','conexion.js','menu-principal.js'],'sgf-shell.4328.js','SHELL'),pack(['configuracion.js','mapa.js','conexion.js','jszip.min.js','qr-flotas.js','reportes-exportacion.js','sgf-publicador-android.js','aplicacion.js'],'sgf-module.4328.js','MODULO')]
p=out/'index.html';t=p.read_text();t=re.sub(r'<script src="configuracion\.js[^>]*></script>\s*<script src="conexion\.js[^>]*></script>\s*<script src="acceso\.js[^>]*></script>','<script src="sgf-login.4328.js?v=web4328-geocerca-checkin-perfil"></script>',t);p.write_text(t)
p=out/'main.html';t=p.read_text();t=re.sub(r'<script src="configuracion\.js[^>]*></script>\s*<script src="conexion\.js[^>]*></script>\s*<script src="menu-principal\.js[^>]*></script>','<script src="sgf-shell.4328.js?v=web4328-geocerca-checkin-perfil"></script>',t);p.write_text(t)
for name in [x for x in htmls if x not in ('index.html','main.html')]:
    p=out/name;t=p.read_text()
    for js in ['configuracion.js','mapa.js','conexion.js','jszip.min.js','qr-flotas.js','reportes-exportacion.js','sgf-publicador-android.js']:
        t=re.sub(rf'\s*<script src="{re.escape(js)}[^>]*></script>','',t)
    t=re.sub(r'<script src="aplicacion\.js[^>]*></script>','<script src="sgf-module.4328.js?v=web4328-geocerca-checkin-perfil"></script>',t)
    p.write_text(t)
(out/'00_LEEME_SUBIR_ESTOS_ARCHIVOS.txt').write_text('''SGF WEB 4.3.28 - PRODUCCION SEGURA PLANA\n\nSuba TODOS estos archivos juntos en la raíz del sitio. No se requieren subcarpetas.\n\nProtecciones: módulos directos vuelven al login; ticket efímero por pestaña; validación API antes de abrir módulos; modo local deshabilitado en producción; postMessage limitado al iframe/origen; identidad y permisos solo se aceptan desde la API; JavaScript de negocio empaquetado/ofuscado sin source maps.\n\nPublicación Android: si se interrumpe la carga, la Web consulta el estado del almacenamiento seguro y continúa desde el último byte confirmado. Si el archivo terminó de cargarse pero se perdió la respuesta final, el sistema lo recupera por su identificador sin duplicarlo. La confirmación admite hasta 180 segundos.\n\nNota: en un hosting estático el código que ejecuta el navegador nunca puede ser 100% secreto. La seguridad real está en la API, que valida sesión, rol, permisos y alcance.\n''',encoding='utf-8')
lines=['SGF WEB PRODUCCION SEGURA PLANA 4.3.28','']+[f'{d} | SHA256 {h} | fuente {a} | distribuido {b}' for d,h,a,b in bundles]
(out/'MANIFIESTO_SEGURIDAD_WEB_4.3.28.txt').write_text('\n'.join(lines)+'\n')
